%To compare protocols across subjects
...
function displayAperiodicDataForSubjectGroupAcrossProtocols(subjectNameLists,protocolName,analysisChoice,protocol2,badEyeCondition,badTrialVersion,badElectrodeRejectionFlag,freqRangeList,axisRangeList,cutoffList,useMedianFlag,hAllPlots,pairedDataFlag,groupType)
...
if ~exist('groupType','var');             groupType = 'Meditators';     end
...
subjectListPos = strcmp(groupType,titleStr);% 1: meditators, 2: Controls
...
if strcmp(refChoice,'none')
    paramLim = {[0.5 2],[-1.5 3] [0 1.5]};
    cLimsTopo{1} = [0 3];  %exponent
    cLimsTopo{2} = [-1 2]; %offset
    cLimsTopo{3} = [-1.5 1.5]; %Power
else
    paramLim = {[-0.5 0],[-1.5 1] [0.5 3.5]};
    cLimsTopo{1} = [-0.5 0.5];  %exponent
    cLimsTopo{2} = [-1 1]; %offset
    cLimsTopo{3} = [-2 2]; %Power
end
%nanValue = 0; % to omit nan in topoplots
freqsToAvoid = [48  50];

%%%%%%%%%%%%%%%%%%%%%%% details and flags %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
parameterName = [{'Slope'} {'Offset '} {'Power'}];
refType = 'unipolar';
...
protocolPos(1) = find(strcmp(protocolNameList,protocolName));
protocolPos(2) = find(strcmp(protocolNameList,protocol2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Get Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
goodSubjectNameLists = getGoodSubjectNameList(subjectNameLists,freqWidth,refType,badEyeCondition,badTrialVersion,protocolPos(2),protocolPos(1),analysisChoice,badElectrodeRejectionFlag,cutoffNumTrials,pairedDataFlag,saveFolderName);
...
for i=1:2 %protocol Pos
    powerDataTMP=[];    exponentDataTMP = [];   offsetDataTMP = [];
    powerDataRefTMP=[]; exponentDataTMPRef = [];   offsetDataTMPRef = [];


    for j=1:length(subjectNameLists{subjectListPos})
        subjectName = subjectNameLists{subjectListPos}{j};
        ...
        if ~isfile(fileName)
            ...
            [tmpPower,tmpExponent,tmpOffset] = getPowerData(tmpData,protocolPos(i),analysisChoice,badElectrodeRejectionFlag,cutoffNumTrials);
    ...
    freqPos = setdiff(find(freqVals>freqRangeList{3}(1) & freqVals<freqRangeList{3}(2)),find(freqVals>=freqsToAvoid(1) & freqVals<=freqsToAvoid(2)));
    paramData{3}{i} = squeeze(log10(sum(powerDataTMP(:,freqPos,:),2)));
    ...
    if ~isempty(protocolPosRef)
        paramDataRef{3}{i} = squeeze(log10(sum(powerDataRefTMP(:,freqPos,:),2)));
    end
...
for i=1:2
    ...
    title(protocolNameList{protocolPos(i)},'color',displaySettings.colorNames(i,:));
    if i==1
        ylabel('Bad subjects (%)');
    end
...
for i=1:numElecGroups
    ...
    for j=1:2  % subjects
        ...
        text(30,yLimsPSD(2)-0.5*j,[titleStr{subjectListPos} protocolNameList{protocolPos(j)} '(' num2str(size(meanPSDData{j},1)) ')'],'color',displaySettings.colorNames(j,:),'parent',hPSD(i));
    end
    ...
    for jFreq=1:length(tmpData.freq_range)
        for k=1:2
            line([tmpData.freq_range{jFreq}(k) tmpData.freq_range{jFreq}(k)],yLimsPSD,'color',freqRangeColors(jFreq,:),'parent',hPSD(i));
             line([freqRangeList{3}(k) freqRangeList{3}(k)],yLimsPSD,'color',[1 0.5 0.5],'parent',hPSD(i));
        end
    end
    ...
    for iParam  =1:length(paramData)
        ...
        if pairedFlag
            ...
            er = errorbar(x,y,err,err,'LineWidth',1.5);
        end
        ...
        set(hPower(iParam,i),'XTick',[1 2],'XTickLabel',[{protocolNameList{protocolPos(1)}} {protocolNameList{protocolPos(2)}}],'TickLength',[0.02, 0.02],'YTick',0:1:2,'TickDir','out','LineWidth',1,'FontWeight','bold','Box','off');
